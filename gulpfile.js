...
gulp.task('build-sass', function() {
	var plugins = [autoprefixer({ browsers: ['last 1 version'] }), cssnano(), discardDuplicates()];

	return (
		gulp
			.src(['vendor/css/normalize.css', 'scss/vars.scss', 'scss/*.scss', 'scss/*.css'])
...
gulp.task('default', function() {
	runSequence('transport-vendor-js', 'transport-vendor-css', 'build-sass', 'concat-js');
});
