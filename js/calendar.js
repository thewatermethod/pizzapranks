if (document.querySelector('#calendarApp')) {
	...
	var calendarApp = new Vue({
		...
		methods: {
				...
				fetch(url)
					...
					.then(function(pixels) {
						for (var index = 0; index < pixels.length; index++) {
							...
							if (pixel.meta.pp_upload_date != '') {
								/** all this gobbledegook is to make it a string moment can digest */
								var bobTheCat = pixel.meta.pp_upload_date.split('/');

								var month = bobTheCat[0];
								...
								date = bobTheCat[2] + '-' + stringMonth + '-' + stringDay;
							}
							...
							for (var x = 0; x < self.days.length; x++) {
								...
								if (moment(day.moment).isSame(moment(date), 'day')) {
									day.pixel = pixel;
								}
							}
						}
			...
			previous: function(selectedPixel) {
				event.stopPropagation();

				var newKey = this.selectedPixel.index - 1;

				if (this.pixels[newKey]) {
					this.selectedPixel = false;
					this.selectedPixel = {};
					var pixel = this.pixels[newKey];
					...
					this.selectedPixel.index = pixel.index;
				}
			},
			next: function(selectedPixel) {
				event.stopPropagation();
			...
			recalculateCalendar: function() {
				...
				for (var index = 0; index < days; index++) {
					...
					var momentDay = moment(
						dayOfMonth + ' ' + this.currentMonth + ' ' + this.currentYear + ' 00:00 EST'
					).format();
